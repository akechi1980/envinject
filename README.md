...
### 为何要把敏感信息单独管理？（AI 时代更必要）

在 AI 时代，代码会频繁与 AI 助手、在线工具和协作者接触：你把片段贴进聊天、让 Copilot 补全、或把整个仓库交给 AI 做重构。若 API 密钥、数据库连接串、`.env` 等写在项目里或随代码一起提交，很容易在一次复制粘贴或一次“分享整个项目”时泄露。**把敏感配置从项目目录中剥离，统一放在本机单独存储**（例如用 envinject 按项目维护在用户配置目录），可以让你：

- **放心把代码交给 AI**：项目里只保留“从环境变量读取”的写法，不包含真实密钥或地址；
- **安全地开源或分享**：仓库里无需 `.env` 或占位配置，别人 clone 后用自己的 envinject 配置即可运行；
- **降低误泄露面**：敏感信息只存在于本机一处，不随 git 提交、不随截图或录屏进入对话与日志。

envinject 不把配置写在项目下，而是写在系统配置目录中，运行命令时再注入，正是为了在“本地单机、自己管理”的前提下，把敏感信息与代码彻底分开。
